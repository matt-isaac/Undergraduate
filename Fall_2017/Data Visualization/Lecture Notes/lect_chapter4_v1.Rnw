...
# This typically only works for two categories. 
@
...
p1 <- p1 + geom_bar(stat = "identity") + 
  ggtitle("Overall admission percentage") + ylim(0, .5) 
p1
@
...
# Category of females admitted is underfilled, and females rejected is overfilled. 
# Males admitted is overfilled, males rejected is underfilled
@
...
# men are overadmitted in A and B, but underadmitted in C-F. 
# Women were highly underadmitted in A, B, and F. However, they are overadmitted in C, and fairly represented in C and D. 
# This can be more advanced with categorical data analysis. More potential with additional arguments
@
...
library(ggmosaic)
library(tibble)
countsToCases <- funtion(x, contcol = "Freq"){
  inx <- repint(seq_len(nrow(x)), x[[countcol]])
  x[[countcol]] <- NULL
  x[idx]
}
...
#width represent total number of applicants
@
